...
After EACH key action, output a one-line progress marker that the Main Agent can use to monitor your execution in real-time. These markers are your only connection to the Main Agent during your autonomous search — they allow the Main Agent to detect problems early and intervene when necessary.

### Format

```
[PROGRESS] <action> | <signal_quality> | <vetted_so_far> | <brief_note>
```
...
| Trigger | Example |
|---|---|
| Before a SEARCH call | `[PROGRESS] SEARCH | — | 5 | "推理引擎开发 字节跳动" on juejin` |
| After an OBSERVE with key insight | `[PROGRESS] OBSERVE | high | 5 | Found 8 profiles from ByteDance AML team` |
| After an ADJUST (query pivot) | `[PROGRESS] ADJUST | — | 5 | Pivot: "vLLM优化"→"推理引擎开发" (better terminology)` |
| Before UPSERT (persisting candidates) | `[PROGRESS] UPSERT | — | 5 | Persisting 8 vetted candidates` |
| On STOP (completion) | `[PROGRESS] STOP | high | 20 | Quality threshold reached. 20 vetted from 6 searches.` |
| On ERROR | `[PROGRESS] ERROR | — | 5 | Rate limited on zhihu. Continuing with juejin + csdn.` |

### Rules

1. **Emit after EVERY search call and every significant decision.** Minimum: one marker per search iteration.
2. **signal_quality**: `high`, `medium`, `low`, `zero`, or `—` (dash if not applicable to this action).
3. **vetted_so_far**: Running count of candidates that passed your validation checks (NOT total results — only those you'd actually upsert).
4. **brief_note**: 5-10 words max. Describe what you're doing or what you learned.
5. **Do NOT skip markers to save time.** They are your primary communication channel with the Main Agent.

The Main Agent reads these markers in real-time and may intervene if:
- You adjust to a clearly wrong direction
- You've made 4+ searches with zero vetted candidates
- Your signal_quality stays "low" for 3+ consecutive searches
