...
After EACH key action, output a one-line progress marker that the Main Agent can use to monitor your enrichment cycles in real-time.

### Format

```
[PROGRESS] <action> | <evidence_count> | <focus_areas_done> | <brief_note>
```
...
| Trigger | Example |
|---|---|
| After INITIALIZE | `[PROGRESS] INIT | 3 | 1/5 | Starting enrichment for 王磊. Existing evidence: 3 items.` |
| Before GATHER (each cycle) | `[PROGRESS] GATHER | 3 | 1/5 | Cycle 2: searching zhihu for profile, csdn for blog posts.` |
| After attaching evidence | `[PROGRESS] ATTACH | 5 | 2/5 | Attached GitHub profile evidence (Tier 2). 15 repos, ByteDance.` |
| After ASSESS (each cycle) | `[PROGRESS] ASSESS | 5 | 2/5 | Cycle 2 done. Extraction quality 0.6. Gaps: career_timeline.` |
| On REPRIORITIZE | `[PROGRESS] REPRIORITIZE | 5 | 2/5 | Continuing to Cycle 3. Targeting career_timeline on LinkedIn.` |
| On STOP (completion) | `[PROGRESS] STOP | 8 | 5/5 | All focus areas covered. 8 evidence items. Chinese presence: confirmed.` |
| On ERROR | `[PROGRESS] ERROR | 5 | 2/5 | zhihu anti-bot block. Skipping zhihu, continuing with other sources.` |

### Rules

1. **Emit after every cycle step.** Minimum: one marker per cycle (INIT → GATHER → ASSESS → REPRIORITIZE).
2. **evidence_count**: Running total of evidence items attached so far.
3. **focus_areas_done**: `<completed>/<total>` format (e.g., "2/5").
4. **brief_note**: 5-10 words describing the current action or key finding.

The Main Agent reads these markers and may intervene if enrichment stalls (no new evidence for 2+ cycles).
