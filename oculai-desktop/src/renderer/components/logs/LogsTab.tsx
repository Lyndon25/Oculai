...
export function LogsTab() {
  ...
  const getIcon = (msg: (typeof messages)[number]) => {
    if (msg.isError) return <AlertTriangle className="h-3.5 w-3.5 text-error" aria-hidden="true" />;
    if (msg.isThinking) return <Brain className="h-3.5 w-3.5 text-purple-500" aria-hidden="true" />;
    if (msg.role === "tool") return <Wrench className="h-3.5 w-3.5 text-warning" aria-hidden="true" />;
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="flex items-center border-b border-rule bg-surface px-4 py-2.5">
        <span className="flex items-center gap-2.5 text-[13px] font-semibold text-ink tracking-tight">
          <Terminal className="h-4 w-4" aria-hidden="true" />
          Agent / System Logs
          <span className="font-mono text-[11px] font-medium text-ink-muted bg-surface-hover rounded-full px-2 py-0.5">
            {messages.length}
          </span>
        </span>
      </div>
            ...
            {messages.map((msg, i) => (
              <div
                ...
                className={cx(
                  ...
                  msg.isError
                    ? "border-error-muted bg-error-soft/50"
                    : msg.isThinking
                      ? "border-purple-100 bg-purple-50/40"
                      : msg.role === "tool"
                        ? "border-warning-muted bg-warning-soft/40"
                        : msg.role === "system"
              ...
              >
                <div className="mb-1 flex items-center gap-2">
                  ...
                  {msg.toolName && (
                    <span className="truncate text-accent text-[11px]">{msg.toolName}</span>
                  )}
                </div>
                <div
                  className={cx(
                    "whitespace-pre-wrap break-words pl-5 leading-[1.6] text-[12px]",
                    msg.isThinking
